"""
Variant v_100m_sql_gb: 100-min sessions, DuckDB SQL features, gradient boosted trees.
...
def user_features(
    ...
    """Sessionize with 100-min inactivity gap (DuckDB SQL), compute per-user features."""
    import duckdb
            ...
            SELECT *,
                CASE WHEN
                    epoch(event_time_parsed - LAG(event_time_parsed)
                        OVER (PARTITION BY user_id ORDER BY event_time_parsed)) / 60.0 > 100
...
def user_predictions(
    ...
    """Train gradient boosted trees, predict conversion probability for each user."""
    import polars as pl
