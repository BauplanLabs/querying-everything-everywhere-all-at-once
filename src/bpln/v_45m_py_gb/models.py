"""
Variant v_45m_py_gb: 45-min sessions, Polars features, gradient boosted trees.
...
def user_features(
    ...
    """Sessionize with 45-min inactivity gap (Polars), compute per-user features."""
    import polars as pl
    ...
    obs = obs.with_columns(
        (pl.col("event_time_parsed").diff().over("user_id").dt.total_minutes() > 45)
        .fill_null(True)
        .alias("new_session")
    )
    obs = obs.with_columns(
